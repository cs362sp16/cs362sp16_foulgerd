...
int main(int argc, char **argv) {
	...
	struct gameState state;
        ...
        //int k[10] = {adventurer, gardens, embargo, village, feast, mine, cutpurse, 
  ...
  //while (!isGameOver(p)) {
	...
	while(i<numHandCards(p)){
      ...
      if (stewardPos != -1) {
              printf("0: steward played from position %d\n", stewardPos);
              playCard(stewardPos, 1, -1, -1, p);
      }


      if ((money >= 8) && (numProvince < 15)) {
        printf("0: bought province\n"); 
        buyCard(province, p);
        numProvince++;
      }
      else if ((money >= 6) && (numGold < 30)) {
        printf("0: bought gold\n"); 
        buyCard(gold, p);
        numGold++;
      }
      else if ((money >= 3) && (numStewards < 1)) {
              printf("0: bought steward\n");
              buyCard(steward, p);
              numStewards++;
      }
      else if ((money >= 4) && (numBarons < 1)) {
              printf("0: bought baron\n");
              buyCard(baron, p);
              numBarons++;
      }
      else if ((money >= 4) && (numFeasts < 1)) {
	...
	while(i<numHandCards(p)){
      ...
      else if ((money >= 4) && (numCutpurses < 1)) {
              printf("1: bought cutpurse\n");
              buyCard(cutpurse, p);
              numCutpurses++;
      }
      else if ((money >= 5) && (numTributes < 1)) {
